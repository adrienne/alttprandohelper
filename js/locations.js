(function(window) {
    ...
    var keysanity_encounters = update(encounters, {
        agahnim: { $merge: {
            ...
            is_progressable: function(items, model) {
                // Todo: combine
                // Hyrule Castle Tower Foyer
                var foyer = items.sword >= 2 || items.cape ? 'available' : 'unavailable';
                // Hyrule Castle Tower Maze
                var maze = model.regions.castle_tower.keys > 0 && (items.sword >= 2 || items.cape) ?
                    items.lantern ? 'available' : 'dark' :
                    'unavailable';
                return 'unavailable';
            }
    ...
    var keysanity_chests = update(chests, {
        mimic: { $merge: {
            is_available: function(items, model) {
                // Todo: verify
                var state = chests.mimic.is_available.call(items, model);
                if (state) return state;
